...
app.post("/add-book", upload.single("img_url_book"), async (req, res) => {
  // req.body will contain the text fields, and req.file will contain the file
  const { name, author, price } = req.body;
  const file = req.file;
  // console.log(req.body);
  // console.log(file);

  if (!name || !author || !price || !file) {
    return res
      .status(400)
  ...
  try {
    ...
    // Create a new book instance using your model logic here
    const newBook = new Book({
      name,
      author,
      price: Number(price), // Ensure price is a number
      img_url_book: imageUrl,
    });
    //console.log(newBook);
